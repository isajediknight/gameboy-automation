...
def load_game(emulator: MGBAEmulator) -> UltraVioletGame:
    emulator.launch(ProjectPaths.ULTRAVIOLET_ROM)
    ...
    for _ in range(3):
        ...
        emulator.press(Button.START, duration_seconds=0.2)
        time.sleep(1)
    ...
    game = UltraVioletGame(session=emulator)
...
def return_to_party(
    ...
    print("Returning to Pokémon party screen...")
...
def scan_eevee(
    ...
    print(f"{slot.name}: {pokemon}")

    assert pokemon.species == "EEVEE"
    assert pokemon.level == 5

    print(f"{slot.name} Eevee detected successfully!")
...
def main() -> None:
    ...
    try:
        ...
        print()
        print("All 6 Level 5 Eevee slots detected successfully!")
